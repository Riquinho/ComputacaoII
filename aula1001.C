...
main (int argc, char *argv[ ])
{
	...
	/* Faz conversao UNIX-DOS armazenando o resultado no arquivo temporario */
	while ((fgets (buffer, COMPRIMENTO_MAXIMO_BUFFER + 1 + 1, arquivoOriginal)) != NULL)
	{
		/* Conjunto dentro de uma linha ou fim da ultima linha sem '\n'. */
		if (buffer [strlen (buffer) - 1] != '\n')
			fprintf (arquivoTemporario, "%s", buffer);
		/* Final das linhas contendo '\n' no buffer. */
		else
		{
	...
	printf ("-------------------------------------------\n");
	printf (" Conversao UNIX-DOS realizada com sucesso!\n\n");
	printf (" (DOS) Arquivo: \"%s\"\n", argv [1]);
	printf (" (UNXI) Backup: \"%s\"\n", nomeArquivoBackup);
	printf ("-------------------------------------------\n");
